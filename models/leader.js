...
const Leader = {
    ...
    getIndividualTenGameBySeasonStoreDivisionId: async (paramsObj) => {
        try {
            const setTieValueVariables = 'SELECT r1.data, r2.data INTO @tie_value_num_leaders, @tie_value_num_leaders_plus_one FROM (SELECT IFNULL((SELECT (g1+g2+g3+g4+g5+g6+g7+g8+g9+g10) as data FROM results WHERE season_id=@season_id && player_id!=100 && store_id=@store_id && division_id=@division_id GROUP BY player_id, week_id, team_id, player_num ORDER BY data DESC LIMIT 19, 1), 1) AS data) AS r1, (SELECT IFNULL((SELECT (g1+g2+g3+g4+g5+g6+g7+g8+g9+g10) as data FROM results WHERE season_id=@season_id && player_id!=100 && store_id=@store_id && division_id=@division_id GROUP BY player_id, week_id, team_id, player_num ORDER BY data DESC LIMIT 20, 1), 0) AS data) AS r2;';
